...
def test_end_to_end_complex():
    query = "Comprehensive analysis of Solid State Battery technology: recent breakthroughs, key players, and realistic timeline to commercialization."
    logger.info(f"Starting end-to-end COMPLEX test with query: '{query}'")
    ...
    for event in builder.stream(query):
        ...
        for node_name, updates in event.items():
            ...
            if "stats" in updates:
                ...
                # new_stats is a list of dicts. In our case usually just one per step.
            ...
            # Update final state tracker (still useful for last message)
            final_state = updates
    ...
    # If still empty, try the last message from the chain (Supervisor's FINISH)
    ...
    # Save JSON config
    output_filename = "complex_test_output.json"
    with open(output_filename, "w") as f:
        f.write(json_str)
    logger.info(f"Output saved to {output_filename}")
    
    # Save Full Report
    report_filename = "complex_test_report.md"
